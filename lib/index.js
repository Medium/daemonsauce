...
function openLogFile() {
    ...
    logToFile("[Restarting log at " + new Date() + ".]\n");
...
function startLogRotation() {
    ...
    function arrangeCallback() {
        ...
        startOfTomorrow.setUTCSeconds(30);
    ...
    function rotateCallback() {
        if (logDir && logFileName && path.existsSync(logFileName)) {
            doRename();
        }
    ...
    function dateString(when) {
        when = new Date(when);
        var year = yesterday.getUTCFullYear();
        var month = yesterday.getUTCMonth() + 1; // +1 because it's zero-based
