...
function parseDaemonArguments(argv) {
    ...
    if (modeIndex < 0) {
	throw new Error("Missing --daemon argument.");
    }
...
function loggingSetup() {
    ...
    // Set up for using syslog. TODO: Change "user" to "daemon" below
    // once <https://github.com/melor/node-posix/pull/1> is resolved.
    posix.openlog(productName, {cons: true, pid: true}, "user");
...
/**
 * Perform the "usual" daemon setup, based on the given arguments of
 * product name, logging directory, and run directory. All three are
 * required.
 */
function usualSetup(productName, logDir, runDir) {
